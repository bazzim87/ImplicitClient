...
function authenticate(client, pcEnvironment) {
    ...
    const platformEnvironment =
        pcEnvironment === "localhost" ? "mypurecloud.de" : pcEnvironment;
    /*
     ...
     */
    const pcOAuthClientIds = {
        "mypurecloud.de": "affa3a5c-1f51-4611-8867-1af88db9ef29",
    };
    const clientId = pcOAuthClientIds[platformEnvironment];
    if (!clientId) {
        const defaultErr =
            platformEnvironment +
            ": Unknown/Unsupported Genesys Cloud Environment";
        const localErr = `
